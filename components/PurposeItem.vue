<template>
  <div class="PurposeItem">
    <div class="icon">
      <Seasonal v-if="data.id === 'seasonal'" />
      <Congratulations v-if="data.id === 'congratulations'" />
      <HelloBaby v-if="data.id === 'hellobaby'" />
      <Love v-if="data.id === 'love'" />
      <Birthday v-if="data.id === 'birthday'" />
      <ThankYou v-if="data.id === 'thankyou'" />
      <Wedding v-if="data.id === 'wedding'" />
      <WorkPlace v-if="data.id === 'workplace'" />
    </div>
    <div class="title">{{ data.title }}</div>
  </div>
</template>
<script>
import Seasonal from '~/components/icons/Seasonal'
import Congratulations from '~/components/icons/Congratulations'
import HelloBaby from '~/components/icons/HelloBaby'
import Love from '~/components/icons/Love'
import Birthday from '~/components/icons/Birthday'
import ThankYou from '~/components/icons/ThankYou'
import Wedding from '~/components/icons/Wedding'
import WorkPlace from '~/components/icons/WorkPlace'

export default {
  components: {
    Seasonal,
    Congratulations,
    HelloBaby,
    Love,
    Birthday,
    ThankYou,
    Wedding,
    WorkPlace
  },
  props: {
    data: {
      type: Object,
      required: true
    }
  },
  computed: {
    iconUrl: function() {
      return require(`~/assets/img/icons/${this.data.id}.svg`)
    }
  }
}
</script>

<style scoped lang="scss">
.PurposeItem {
  flex: 0 0;
  display: table-cell;
  padding: 8px;
  &:hover {
    opacity: 0.9;
    a:hover {
      opacity: 0.9;
    }
  }
  .title {
    font-size: 10px;
  }
}
</style>
